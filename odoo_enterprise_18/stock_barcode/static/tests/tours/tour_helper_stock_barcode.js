...
export function assertLinePackage(lineOrIndex, packageName) {
    const line = _getLineOrFail(lineOrIndex, "Can't check line's package");
    const linePackage = line.querySelector(".package")?.innerText;
    assert(linePackage, packageName, "Not the expected package");
}
